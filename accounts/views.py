...
from .forms import RegisterForm, WelcomeForm
...
def welcome(request):
    if request.method == 'POST': # 회원가입 데이터 입력 완료 상황
        user_form = WelcomeForm(request.POST) #request.POST-사용자가입력한 값
        if user_form.is_valid(): #데이터형식이맞는지 확인, validation 호출
            new_user = user_form.save(commit=False) #해당 form 모델의 인스턴스를 얻어옴
            new_user.set_password(user_form.cleaned_data['password']) #password할당
            new_user.save()
        ...
        user_form = WelcomeForm()

    return render(request, 'registration/welcome.html', {'form':user_form})
